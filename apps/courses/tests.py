...
class MaterialModelTests(TestCase):
    ...
    def test_video_link_without_url_is_invalid(self):
        material = Material(course=self.course, title="Lecture", type=Material.MaterialType.VIDEO_LINK)
        with self.assertRaises(ValidationError):
            material.clean()
...
class CoursePermissionAPITests(APITestCase):
    ...
    def test_non_author_cannot_update_course(self):
        ...
        response = self.client.patch(reverse("course-detail", args=[course.id]), {"title": "Hacked"})
    ...
    def test_author_can_update_own_course(self):
        course = Course.objects.create(title="Django basics", author=self.author, category=self.category)
        self.client.force_authenticate(self.author)

        response = self.client.patch(reverse("course-detail", args=[course.id]), {"title": "Updated title"})
...
class MaterialAPITests(APITestCase):
    def setUp(self):
        self.author = create_user(email="author@example.com", username="author")
        self.other = create_user(email="other@example.com", username="other")
        self.course = Course.objects.create(title="Django basics", author=self.author, is_published=True)
    ...
    def test_author_can_add_pdf_material(self):
        self.client.force_authenticate(self.author)
        pdf = SimpleUploadedFile("slides.pdf", b"%PDF-1.4 fake pdf content", content_type="application/pdf")
